...
const ToastItem = ({ toast, onDismiss }) => {
  ...
  const borders = {
    ...
    info: 'border-blue-500/30 hover:border-blue-500/50 shadow-blue-500/10',
  };
  ...
  return (
    <div
      ...
      className={`pointer-events-auto relative w-full max-w-sm overflow-hidden rounded-2xl bg-white dark:bg-zinc-950/95 backdrop-blur-xl border ${borders[type]} p-4 shadow-2xl transition-all duration-300 animate-in slide-in-from-bottom-5 fade-in group`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 pr-4">
          ...
          <p className="text-xs text-slate-600 dark:text-zinc-300 mt-1 leading-relaxed break-words font-normal">
            {toast.message}
          </p>
        </div>
        ...
        <button
          ...
          className="text-slate-400 hover:text-slate-900 dark:text-zinc-500 dark:hover:text-white p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-900 transition-colors shrink-0 cursor-pointer"
          title="Dismiss"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-slate-100 dark:bg-zinc-900 overflow-hidden">
        <div

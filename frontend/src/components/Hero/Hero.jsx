...
const Hero = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] rounded-full blur-[140px] opacity-20 pointer-events-none transition-transform duration-700 ease-out"
        style={{
          background: 'radial-gradient(circle, var(--color-accent, #f59e0b) 0%, rgba(168,85,247,0.4) 60%, transparent 80%)',
          transform: `translate(calc(-50% + ${mousePos.x * 1.5}px), calc(-50% + ${mousePos.y * 1.5}px))`,
        }}
      />
      <div
        className="absolute bottom-10 right-10 w-96 h-96 rounded-full bg-blue-500/10 blur-[120px] pointer-events-none transition-transform duration-1000 ease-out"
        style={{
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-8 z-10">
        {/* Top Tag Pill */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-semibold text-[var(--color-accent,#f59e0b)] backdrop-blur-md shadow-lg animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <div className="max-w-4xl mx-auto space-y-4">
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-[var(--text-primary,#ffffff)] tracking-tight leading-[1.1] sm:leading-[1.1]">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-[var(--text-secondary,#a1a1aa)] max-w-2xl mx-auto leading-relaxed font-normal pt-2">
            No more passive watching. Adhyaya AI breaks videos into milestone modules with synchronized notes, instant practice quizzes, coding labs, and a RAG AI tutor.
          </p>
        </div>

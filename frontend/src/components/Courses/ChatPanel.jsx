    ...
    while ((match = timestampRegex.exec(str)) !== null) {
      ...
      parts.push(
        <button
          ...
          className="inline-flex items-center gap-1 px-1.5 py-0.5 mx-0.5 text-[10px] font-mono font-bold rounded bg-amber-500/20 text-amber-400 border border-amber-500/30 hover:bg-amber-500/30 transition-colors cursor-pointer"
        >
    ...
    return parts.map((part, pIdx) => {
      ...
      return subParts.map((sub, sIdx) => {
        if (sub.startsWith('**') && sub.endsWith('**')) {
          return <strong key={`${pIdx}-${sIdx}`} className="font-semibold text-zinc-100">{sub.slice(2, -2)}</strong>;
        }
        if (sub.startsWith('`') && sub.endsWith('`')) {
          return (
            <code key={`${pIdx}-${sIdx}`} className="px-1 py-0.5 rounded text-[10px] bg-zinc-800 text-amber-300 font-mono border border-zinc-700">
              {sub.slice(1, -1)}
            </code>
          );
        }
        ...
        if (numList) {
          return (
            <div key={i} className="flex gap-2 items-start">
              <span className="text-amber-400 font-bold flex-shrink-0 w-4">{numList[1]}.</span>
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer ${
          open
            ...
            : 'bg-gradient-to-r from-amber-500 to-amber-600 text-black font-bold shadow-amber-500/20'
        }`}
      ...
      >
        {open ? (
          <>
            <X className="w-4 h-4" />
            <span className="text-xs font-semibold">Close Tutor</span>
          </>
        ) : (
          <>
      ...
      {/* Floating Window */}
      <div
        className={`fixed bottom-22 right-6 z-40 flex flex-col rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 origin-bottom-right border border-zinc-800/80 bg-zinc-950/95 backdrop-blur-xl ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
          {messages.map((msg, idx) => {
            ...
            return (
              ...
              >
                {!isUser && (
                  <div className="w-6 h-6 rounded-md bg-amber-500/20 border border-amber-500/30 flex items-center justify-center shrink-0 mt-0.5">
                ...
                <div className="space-y-1.5 max-w-[85%]">
                  <div
                    className={`p-3.5 rounded-2xl text-xs ${
                      isUser
                        ? 'bg-amber-500/15 border border-amber-500/30 text-zinc-100 rounded-tr-sm'
                    ...
                    {msg.typing ? (
                      ...
                      <>
                        ...
                        {msg.sources && msg.sources.length > 0 && (
                          <div className="mt-3 pt-2 border-t border-zinc-800/80 flex flex-wrap gap-1 items-center">
                            <span className="text-[9px] text-zinc-500 font-semibold uppercase tracking-wider flex items-center gap-1 mr-1">
                              <BookOpen className="w-2.5 h-2.5 text-zinc-500" /> Grounding:
                            </span>
                            {msg.sources.map((s, sIdx) => (
                              <span
                                key={sIdx}
                                className="text-[9px] px-2 py-0.5 rounded-full bg-zinc-800 text-amber-400 border border-zinc-700 font-medium"
                              >
                                {s}
                              </span>
                            ))}
                    ...
                    <div className="flex items-center gap-2 pl-1">
                      ...
                      {onInsertToNotes && (
                        <button
                          ...
                          className="text-[10px] text-zinc-500 hover:text-amber-400 flex items-center gap-1 transition-colors cursor-pointer"
                        >
                          {insertedIndex === idx ? <Check className="w-3 h-3 text-green-400" /> : <PlusCircle className="w-3 h-3" />}
          ...
          {messages.length === 1 && isReady && (
            <div className="pt-2 space-y-1.5">
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold pl-1">
                Suggested Prompts
              </p>
              {SUGGESTIONS.map((s, idx) => (
                <button
                  ...
                  className="w-full text-left p-2 rounded-xl bg-zinc-900/60 border border-zinc-800/80 hover:border-amber-500/40 hover:bg-zinc-800/60 text-zinc-300 hover:text-amber-400 text-xs transition-all flex items-center justify-between group cursor-pointer"
                >
                  <span>{s}</span>
        ...
        <div className="p-3 border-t border-zinc-800 bg-zinc-900/60">
          <div className="flex items-end gap-2 p-2 rounded-xl bg-zinc-950 border border-zinc-800 focus-within:border-amber-500/50 transition-colors">
            <textarea
              ref={inputRef}
              value={input}
              ...
              rows={1}
              className="flex-1 bg-transparent text-xs text-zinc-200 placeholder-zinc-500 resize-none outline-none max-h-24 py-1"

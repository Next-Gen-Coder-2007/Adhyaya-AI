    ...
    return (
      <Navbar>
        <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto space-y-4">
          ...
          <p className="text-sm text-slate-600 dark:text-zinc-400">{error || 'Could not find this course.'}</p>
          <button
    ...
    <Navbar>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={() => navigate('/courses')}
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-slate-200 dark:border-zinc-800 text-slate-700 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-white transition-colors text-xs font-semibold uppercase tracking-wider group cursor-pointer shadow-sm"
          >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-slate-200 dark:border-zinc-800 text-xs font-semibold text-slate-700 hover:text-slate-900 dark:text-zinc-300 dark:hover:text-white transition-colors shadow-sm"
            >
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-100 hover:bg-red-50 dark:bg-zinc-900 dark:hover:bg-red-950/40 border border-slate-200 hover:border-red-300 dark:border-zinc-800 dark:hover:border-red-800/60 text-slate-500 hover:text-red-600 dark:text-zinc-400 dark:hover:text-red-400 transition-colors cursor-pointer shadow-sm"
              title="Delete Course"
            >
        ...
        {/* Hero Card */}
        <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-900 shadow-xl relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-8 relative z-10">
            <div className="space-y-4 max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-[11px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-widest">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800">
                  <span className="text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider block">Modules</span>
                  <span className="text-base font-bold text-slate-900 dark:text-white mt-0.5 block">{course.modules?.length || 0}</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800">
                  <span className="text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider block">Lectures</span>
                  <span className="text-base font-bold text-slate-900 dark:text-white mt-0.5 block">{videoCount}</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800">
                  <span className="text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider block">Quizzes</span>
                  <span className="text-base font-bold text-slate-900 dark:text-white mt-0.5 block">{quizCount}</span>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800">
                  <span className="text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider block">Completed</span>
            ...
            <div className="w-full lg:w-72 space-y-4 shrink-0">
              {(() => {
                ...
                return (
                  <div className="aspect-video rounded-2xl overflow-hidden border border-slate-200 dark:border-zinc-800 relative group shadow-xl">
                    <img
                      src={thumb}
                      alt={course.title}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Course Syllabus</h3>
              <p className="text-xs text-slate-500 dark:text-zinc-400">Explore modules, learning nodes, and assignments</p>
            </div>
          ...
          <div className="space-y-3">
            {course.modules?.map((module, idx) => {
              ...
              return (
                <div
                  key={module.id}
                  className="rounded-2xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-900 overflow-hidden transition-all shadow-sm"
                >
                  <button
                    onClick={() => toggleModule(module.id)}
                    className="w-full p-4 flex items-center justify-between text-left hover:bg-slate-50 dark:hover:bg-zinc-900/40 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-7 h-7 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-500 font-bold text-xs flex items-center justify-center font-mono border border-amber-500/20">
                        {idx + 1}
                      </span>
                      <div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-200 dark:border-zinc-900 p-3 bg-slate-50/80 dark:bg-zinc-900/40 space-y-2">
                      {modSections.map((section) => (
                        <div
                          key={section.id}
                          onClick={() => navigate(`/courses/${id}/learn`)}
                          className="flex items-center justify-between p-2.5 rounded-xl hover:bg-white dark:hover:bg-zinc-800/60 border border-transparent hover:border-slate-200 dark:hover:border-zinc-700/60 transition-colors cursor-pointer text-xs"
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            {getSectionIcon(section.type)}
                            <span className="text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white truncate font-medium">
                              {section.title}
                            </span>
                          </div>

                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-slate-500 dark:text-zinc-400 uppercase tracking-wider font-mono">
            ...
            <div className="w-full max-w-md p-6 rounded-3xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-2xl space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  disabled={deleting}
                  className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={deleting}
                  className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-500 transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
                >
